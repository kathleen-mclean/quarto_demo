...
diabetes %>%
  ...
  bold(part = "header") %>%
  width(width = c(0.79, 1, 0.56, 1.05, 1, 0.56, 1.05)) 
...
-   We also recommend the [`gt` package](https://gt.rstudio.com/index.html) on its own as one of the best packages for table generation and formatting. `gt` now supports output to .docx, but it is currently having an issue outputting to MS Word format in Quarto (see [https://github.com/quarto-dev/quarto-cli/issues/7151](https://github.com/quarto-dev/quarto-cli/issues/7151)).
