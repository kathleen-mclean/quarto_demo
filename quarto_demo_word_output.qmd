...
format: 
  docx:
    reference-doc: word_styles_reference_doc.docx
execute: 
  echo: false
  warning: false
  message: false
...
This document is a demonstration of Quarto. Quarto enables you to weave together content and executable code into a finished document. To learn more about Quarto see [the Quarto guide](https://quarto.org/docs/guide/){.uri}. Quarto is the next-generation of R Markdown, and includes new features and capabilities while at the same being able to render most existing .Rmd files without modification. In both Quarto and R Markdown documents, text content is written in [markdown](https://www.markdownguide.org/), a lightweight markup language that you can use to add formatting elements to plaintext text documents.
...
diabetes %>%
  ...
  bold(part = "header") %>%
  # use this instead of autofit() for word output
  set_table_properties(layout = "autofit") %>%
  set_caption(caption = "Variable summary statistics by diabetes status")
...
-   To use `gtsummary` table output when outputting to Word format, you can convert to a `flextable` object using the `as_flex_table()` function.

-   We also recommend the [`gt` package](https://gt.rstudio.com/index.html) on its own as one of the best packages for table generation and formatting, but it does not currently support Word format (I've heard will soon though!).
...
model_fit %>%
  ...
  as_flex_table() %>%
  bold(part = "header") %>%
  set_caption(caption = "Summary of regression output for the model predicting diabetes status using all available diagnostic measurements")

```

::: callout-note
Note that automatic table numbering and cross-references are not working perfectly when using `flextable` and Word format. I am using the `run_reference()` function from the `officer` package as a workaround for table cross-references in the meanwhile. This issue should be fixed in subsequent releases of Quarto and `flextable`.
:::
